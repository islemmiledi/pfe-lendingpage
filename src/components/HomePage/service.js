      ...
      <div className="max-container padding-hero-y padding-x">
        ...
        <div className="grid grid-cols-3 cursor-pointer gap-10 place-items-center overflow-hidden max-lg:grid-cols-2 max-sm:grid-cols-1">
          {services.map((val) => (
            <div
              key={val.label}
              className="relative group bg-white h-[350px] py-20 pb-9 text-center flex flex-col items-center justify-center max-xl:pt-16"
            >
              {/* Contenu de la carte */}

              <div>
                <p className="group-hover:text-[#f04e3c] flex justify-center mb-10 text-5xl">
                  {val.icon}
                </p>
